...
router.post('/projects/:projectId/tasks', authRequired, async(req, res) => {
    ...
    try {
        ...
        const proj = p.rows[0];
        if (req.auth.role !== 'super_admin' && req.auth.tenantId !== proj.tenant_id) return res.status(403).json(fail('Project does not belong to tenant'));
       ...
       RETURNING id, project_id, tenant_id, title, description, status, priority, assigned_to, due_date, created_at`, [projectId, proj.tenant_id, title, description || null, priority, assignedTo || null, dueDate || null]
        );
...
router.get('/projects/:projectId/tasks', authRequired, async(req, res) => {
    ...
    try {
        ...
        if (status) { where += ` AND t.status=$${idx}`;
            params.push(status);
            idx++; }
        if (assignedTo) { where += ` AND t.assigned_to=$${idx}`;
            params.push(assignedTo);
            idx++; }
        if (priority) { where += ` AND t.priority=$${idx}`;
            params.push(priority);
            idx++; }
        if (search) { where += ` AND LOWER(t.title) LIKE $${idx}`;
...
router.patch('/:taskId/status', authRequired, async(req, res) => {
    ...
    try {
        ...
        const task = t.rows[0];
        if (req.auth.role !== 'super_admin' && req.auth.tenantId !== task.tenant_id) return res.status(403).json(fail('Forbidden'));
...
router.put('/:taskId', authRequired, async(req, res) => {
    ...
    try {
        ...
        let values = [];
        let idx = 1;
        if (title) { fields.push(`title=$${idx++}`);
            values.push(title); }
        if (description !== undefined) { fields.push(`description=$${idx++}`);
            values.push(description || null); }
        if (status) { fields.push(`status=$${idx++}`);
            values.push(status); }
        if (priority) { fields.push(`priority=$${idx++}`);
            values.push(priority); }
        if (assignedTo !== undefined) { fields.push(`assigned_to=$${idx++}`);
            values.push(assignedTo || null); }
        if (dueDate !== undefined) { fields.push(`due_date=$${idx++}`);
